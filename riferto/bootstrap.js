...
const DEFAULT_UPDATE_SOURCE='https://making-lemonade.github.io/riferto/';
...
function normalizeBase(value){
  try{
    const url=new URL(String(value||DEFAULT_UPDATE_SOURCE).trim());
    url.hash='';url.search='';
    if(!url.pathname.endsWith('/'))url.pathname+='/';
    return url.href;
  }catch{return DEFAULT_UPDATE_SOURCE}
}
function updateSourceBase(){return normalizeBase(localStorage.getItem(UPDATE_SOURCE_KEY)||DEFAULT_UPDATE_SOURCE)}
function currentBase(){return new URL('./',location.href).href}
...
async function ensureCurrentServiceWorker(){
  ...
  const existing=registrations.find(reg=>reg.scope===base);
...
async function hardUpdate(targetVersion=null,button=null){
  const b=button||$('#forceUpdateBtn')||$('#settingsUpdateNowBtn');const old=b?.textContent||'';
  ...
  try{
    ...
    const source=normalizeBase(published.sourceBase||updateSourceBase());
    const here=currentBase();
    const sourceUrl=new URL(source),hereUrl=new URL(here);
    const verify=await fetch(new URL(`index.html?verify=${encodeURIComponent(target)}&t=${Date.now()}`,source),{cache:'no-store'});
    const html=await verify.text();
    if(!verify.ok||!html.includes(`v${target}`))throw new Error('La nuova shell non è ancora disponibile all’URL configurato.');
    if(source!==here){
      if(sourceUrl.origin!==hereUrl.origin){
        throw new Error('La nuova URL usa un dominio diverso. Per sicurezza Riferto non sposta automaticamente l’archivio locale tra domini: esporta un backup .riferto e apri la nuova URL dalle Impostazioni.');
      }
      if(b)b.textContent='Apro la nuova URL…';
      const next=new URL(source);next.searchParams.set('release',target);next.searchParams.set('t',Date.now().toString());location.replace(next.toString());return;
    }
    if(b)b.textContent=`Installo v${target}…`;
    await clearRifertoCaches();await installTargetServiceWorker(target);
    if(b)b.textContent='Riavvio…';
    const u=new URL('./',location.href);u.searchParams.set('release',target);u.searchParams.set('t',Date.now().toString());location.replace(u.toString());
...
window.RifertoUpdateSourceBase=updateSourceBase;

(async()=>{await ensureCurrentServiceWorker();if(!isStandalone){showInstallOnly();return}$('#installScreen')?.classList.add('hidden');$('#appShell')?.classList.add('hidden');$('#appShell')?.setAttribute('aria-hidden','true');$('#appShell')?.setAttribute('inert','');$('#bottomNav')?.classList.add('hidden');$('#lockScreen')?.classList.remove('hidden');try{await ensureDatabase();await import(`./setup-security.js?v=${APP_VERSION}`);await import(`./app.js?v=${APP_VERSION}`);await import(`./family-core.js?v=${APP_VERSION}`);await import(`./responsive-family-ui.js?v=${APP_VERSION}`);await import(`./biometric.js?v=${APP_VERSION}`);await import(`./storage-backup.js?v=${APP_VERSION}`);await import(`./backup-package-0132.js?v=${APP_VERSION}`);await import(`./backup-mode-0132.js?v=${APP_VERSION}`);await import(`./reports-search.js?v=${APP_VERSION}`);await import(`./report-view-guard.js?v=${APP_VERSION}`);await import(`./canonical-backfill.js?v=${APP_VERSION}`);await import(`./trend.js?v=${APP_VERSION}`);await import(`./trend-matrix-polish.js?v=${APP_VERSION}`);await import(`./settings-accordion.js?v=${APP_VERSION}`);await import(`./polish-0121.js?v=${APP_VERSION}`);await import(`./report-polish-0122.js?v=${APP_VERSION}`);await import(`./settings-info-0131.js?v=${APP_VERSION}`);await import(`./update-source-0134.js?v=${APP_VERSION}`);await import(`./update-badge-ui.js?v=${APP_VERSION}`);setTimeout(()=>checkForUpdates(),500)}catch(e){console.error(e);$('#lockIntro').textContent='Errore di inizializzazione locale. Chiudi e riapri Riferto.';$('#unlockBtn').textContent='Riprova';$('#unlockBtn').onclick=()=>location.reload();$('#lockError').textContent=e?.message||'Errore di inizializzazione'}})();
