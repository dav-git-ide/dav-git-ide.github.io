...
(async()=>{await ensureCurrentServiceWorker();if(!isStandalone){showInstallOnly();return}$('#installScreen')?.classList.add('hidden');$('#appShell')?.classList.add('hidden');$('#appShell')?.setAttribute('aria-hidden','true');$('#appShell')?.setAttribute('inert','');$('#bottomNav')?.classList.add('hidden');$('#lockScreen')?.classList.remove('hidden');try{await ensureDatabase();await import(`./setup-security.js?v=${APP_VERSION}`);await import(`./app.js?v=${APP_VERSION}`);await import(`./biometric.js?v=${APP_VERSION}`);await import(`./storage-backup.js?v=${APP_VERSION}`);await import(`./reports-search.js?v=${APP_VERSION}`);await import(`./report-view-guard.js?v=${APP_VERSION}`);await import(`./trend.js?v=${APP_VERSION}`);await import(`./settings-accordion.js?v=${APP_VERSION}`);setTimeout(()=>checkForUpdates(),500)}catch(e){console.error(e);$('#lockIntro').textContent='Errore di inizializzazione locale. Chiudi e riapri Riferto.';$('#unlockBtn').textContent='Riprova';$('#unlockBtn').onclick=()=>location.reload();$('#lockError').textContent=e?.message||'Errore di inizializzazione'}})();