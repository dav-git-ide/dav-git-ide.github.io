...
async function registerServiceWorker(version=APP_VERSION){if(!('serviceWorker'in navigator))return null;try{const reg=await navigator.serviceWorker.register(`./sw.js?v=${encodeURIComponent(version)}&t=${Date.now()}`,{updateViaCache:'none'});await reg.update().catch(()=>{});return reg}catch(e){console.warn('SW',e);return null}}
