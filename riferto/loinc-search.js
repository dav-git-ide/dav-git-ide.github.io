...
style.textContent=`
.loinc-autocomplete{position:relative}
.loinc-results{position:absolute;left:0;right:0;top:calc(100% + 6px);z-index:40000;display:none;max-height:280px;overflow:auto;border-radius:18px;border:1px solid rgba(255,255,255,.82);background:rgba(248,251,255,.98);box-shadow:0 18px 45px rgba(31,54,88,.2);backdrop-filter:blur(26px);-webkit-backdrop-filter:blur(26px)}
.loinc-results.open{display:block}
.loinc-result{width:100%;border:0;background:transparent;text-align:left;padding:12px 14px;display:block;color:#10203a}
.loinc-result+.loinc-result{border-top:1px solid rgba(80,104,140,.1)}
.loinc-result strong{display:block;font-size:.92rem}
...
function attach(input){
  ...
  input.dataset.loincAutocomplete='1';
  input.removeAttribute('list');
  const field=input.closest('.field');
  if(!field)return;
  field.classList.add('loinc-autocomplete');
  const results=document.createElement('div');
  results.className='loinc-results';
  field.appendChild(results);

  const render=()=>{
    const q=normalize(input.value.split(' — ')[0]===input.value?input.value:input.value);
    ...
    for(const item of matches){
      ...
      button.addEventListener('click',()=>{
        input.value=`${item.code} — ${nameOf(item)}`;
        input.closest('.measurement-row').dataset.loincCode=item.code;
        const unit=input.closest('.measurement-row').querySelector('.test-unit');
        if(unit&&!unit.value&&item.commonUnits?.[0])unit.value=item.commonUnits[0];
        results.classList.remove('open');
        input.dispatchEvent(new Event('change',{bubbles:true}));
        setTimeout(()=>input.closest('.measurement-row').querySelector('.test-value')?.focus(),60);
      });
  ...
  input.addEventListener('focus',()=>{if(input.value&&!input.value.includes(' — '))render()});
...
(async()=>{
  try{catalog=await fetch(`./loinc-common.json?t=${Date.now()}`,{cache:'no-store'}).then(r=>r.json())}catch(error){console.warn('LOINC search unavailable',error);catalog=[]}
  scan();
