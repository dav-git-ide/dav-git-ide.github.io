...
.app-section[data-section="reports"]>.grid.two{display:none!important}
...
function updateHeroCount(){const count=Number.parseInt(reportCount?.textContent||'0',10)||0;if(heroTitle)heroTitle.textContent=`I tuoi ${count} ${count===1?'referto':'referti'}. Sul tuo dispositivo.`}
...
function resetVisibleCards(){for(const report of [...(reportsList?.querySelectorAll('.report-card')||[])])report.classList.remove('hidden');noResults.classList.add('hidden');syncTrashButtons();setEmptyStateFromCards();updateHeroCount()}
function applyFilters(){compactReportCards();syncTrashButtons();updateHeroCount();const cards=[...(reportsList?.querySelectorAll('.report-card')||[])];setEmptyStateFromCards();if(card.classList.contains('hidden')){resetVisibleCards();return}const q=normalize(textFilter.value);const wantedDate=selectedDateLabel();let visible=0;for(const report of cards){const laboratory=normalize(report.querySelector('.report-meta')?.textContent||'');const date=normalize(report.querySelector('.report-top h3')?.textContent||'');const show=(!q||laboratory.includes(q))&&(!wantedDate||date===wantedDate);report.classList.toggle('hidden',!show);if(show)visible++}const filtering=Boolean(q||dateFilter.value);noResults.classList.toggle('hidden',!filtering||visible>0||cards.length===0)}
...
clearDate.addEventListener('pointerdown',event=>event.preventDefault());
clearDate.addEventListener('click',()=>{dateFilter.value='';dateFilter.blur();document.activeElement?.blur?.();applyFilters()});
