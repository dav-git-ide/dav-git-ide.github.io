...
function render(){const key=examSelect.value;if(!key){output.innerHTML='<div class="trend-empty"><strong>Seleziona un esame</strong><p class="muted">Il trend apparirà qui quando lo stesso esame è presente in uno o più referti.</p></div>';return}const all=observations().filter(o=>o.key===key);if(!all.length)return;const latest=[...all].sort((a,b)=>`${b.date.year}-${String(b.date.month).padStart(2,'0')}-${String(b.date.day).padStart(2,'0')}`.localeCompare(`${a.date.year}-${String(a.date.month).padStart(2,'0')}-${String(a.date.day).padStart(2,'0')}`))[0];const rows=all.filter(o=>o.unit===latest.unit);const ignored=all.length-rows.length;const data=aggregate(rows);const body=viewMode==='chart'?`<div class="trend-chart-wrap">${chartSvg(data)}</div>`:`<div style="overflow-x:auto"><table class="trend-table"><thead><tr><th>Periodo</th><th>Media</th><th>N.</th><th>Min–Max</th></tr></thead><tbody>${data.map(d=>`<tr><td>${d.label}</td><td>${fmt(d.value)} ${latest.unit}</td><td>${d.count}</td><td>${fmt(d.min)}–${fmt(d.max)}</td></tr>`).join('')}</tbody></table></div>`;output.innerHTML=`<div class="trend-head"><div><h3>${latest.name}</h3><div class="trend-unit">LOINC ${latest.code} · ${latest.unit||'unità non indicata'}</div></div><span class="pill">${rows.length} valori</span></div>${body}${ignored?`<div class="trend-note">${ignored} valore/i con unità diversa non sono stati aggregati.</div>`:''}`}
